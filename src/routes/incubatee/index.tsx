...
export const IncubateeDashboard: React.FC = () => {
  ...
  return (
    <div style={{ padding: 24, position: 'relative' }}>
          ...
          <Card title="Pending Interventions">
            <List
              ...
              renderItem={(item) => (
                ...
                >
                  <List.Item.Meta title={item.title} description={`Scheduled Date: ${item.date}`} />
                </List.Item>
              )}
            />
          </Card>
          ...
          <List
            ...
            renderItem={(note) => (
              <List.Item
                ...
                style={{ cursor: 'pointer', backgroundColor: note.read ? '#f0f0f0' : 'white' }}
              >
      ...
      >
        {unreadCount > 0 && <span style={{ position: 'absolute', top: -8, right: -8, background: 'red', borderRadius: '50%', color: 'white', padding: '2px 6px', fontSize: '12px' }}>{unreadCount}</span>}
      </Button>
    </div>
  );
};
