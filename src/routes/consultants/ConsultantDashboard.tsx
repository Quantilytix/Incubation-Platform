...
const getEventStartEnd = (event: any) => {
    const start =
        toJsDate(event.startTime) ??
        toJsDate(event.time) ??
        (dayjs(event.date).isValid() ? dayjs(event.date).toDate() : null)
...
const isUpcomingEvent = (event: any) => {
    ...
    const now = dayjs()
    // If event has an end time, treat it as upcoming if it hasn't ended yet.
    if (end) return dayjs(end).isAfter(now)
    // Otherwise upcoming if start is still in the future (or now)
    return dayjs(start).isAfter(now.subtract(1, 'minute'))
}
...
export const ConsultantDashboard: React.FC = () => {
    ...
    const upcomingEvents = useMemo(() => {
        return [...events]
            ...
            .sort((a, b) => {
                const sa = getEventStartEnd(a).start
                const sb = getEventStartEnd(b).start
                return dayjs(sa || 0).valueOf() - dayjs(sb || 0).valueOf()
            })
    }, [events])
    ...
    const normalizedNotifs = useMemo(() => {
        const role = currentRole || 'consultant'
        ...
        const list = [...notifications]
            ...
            .filter(n => {
                if (notifUnreadOnly && n.readBy?.[role]) return false

                if (!notifSearch.trim()) return true
                const q = notifSearch.toLowerCase()
                const title = (n.interventionTitle || n.title || '').toLowerCase()
                const msg = (n.message?.[role] || '').toLowerCase()
                return title.includes(q) || msg.includes(q)
            })

        return list
    }, [notifications, currentRole, notifUnreadOnly, notifSearch])

    const markAllVisibleAsRead = async () => {
        if (!currentRole) return
        try {
            const unread = normalizedNotifs.filter(n => !n.readBy?.[currentRole])
            if (!unread.length) return message.info('No unread notifications.')
            ...
            setNotifications(prev =>
                prev.map(n =>
                    unread.some(u => u.id === n.id)
                        ? { ...n, readBy: { ...(n.readBy || {}), [currentRole]: true } }
                        : n
                )
            )
            setUnreadCount(0)
            message.success(`Marked ${unread.length} as read.`)
        } catch (e) {
            console.error(e)
    ...
    const confirmBatch = async (
        ...
        return new Promise<void>((resolve, reject) => {
            Modal.confirm({
                title,
                content: `This will ${action} ${count > 1 ? 'all selected assignments' : 'this assignment'
                    } for: ${baseLabel}.`,
                    ...
                    >
                        <Space direction="vertical" style={{ width: '100%' }} size={12}>
                            <Row gutter={8}>
                                ...
                                <Col>
                                    <Button onClick={markAllVisibleAsRead} disabled={!unreadCount}>
                                        Mark all read
                                    </Button>
                                </Col>
                                <Col>
                                    <Button
                                        type={notifUnreadOnly ? 'primary' : 'default'}
                                        onClick={() => {
                            ...
                            {loadingNotifications ? (
                                ...
                                <List
                                    ...
                                    renderItem={(item: any) => {
                                        const isRead = !!item.readBy?.[currentRole || 'consultant']
                                        const created =
                                            toJsDate(item.createdAt) || toJsDate(item.timestamp) || null
                                                ...
                                                <List.Item.Meta
                                                    title={
                                                        <Space>
                                                            <Badge
                                                                ...
                                                                text={
                                                                    <Text strong>{item.interventionTitle || 'Notification'}</Text>
                                                                }
                                                            />
                                                        </Space>
                                                    }
                                                    description={
                                                        <Space direction="vertical" size={2} style={{ width: '100%' }}>
                                                            <Text type="secondary">
                                                                {item.message?.[currentRole || 'consultant'] ||
                                                                    'No message available for your role.'}
                                                            </Text>

                                                            <Text type="secondary" style={{ fontSize: 12 }}>
                                                                {created ? dayjs(created).format('DD MMM YYYY, HH:mm') : '—'}
                                                            </Text>
                                                        </Space>
                                                    }
                                                />
