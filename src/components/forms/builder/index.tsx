...
export default function FormBuilder () {
  ...
  const [baseline, setBaseline] = useState<FormTemplate | null>(null)
  ...
  const selectedField =
    formData.fields.find(f => f.id === selectedFieldId) || undefined
  ...
  const saveTemplate = async (status?: 'draft' | 'published') => {
    ...
    try {
      ...
      setBaseline(payload) // update baseline after successful save
    } catch (e) {
      console.error(e)
  ...
  const loadTemplate = async (templateId: string) => {
    try {
      ...
      const hydrated = { ...tpl, id: templateId }
  ...
  useEffect(() => {
    ...
    else setBaseline(formData) // baseline for a new blank form
            ...
            renderItem={(f, idx) => (
              <List.Item
                ...
                onClick={() => {
                  ...
                  itemRefs.current[f.id]?.scrollIntoView({
                    behavior: 'smooth',
                    block: 'center'
                  })
                }}
              ...
              onClick={() => {
                if (isDirty) {
                  Modal.confirm({
                    ...
                    content:
                      'Do you want to save this form as a draft before going back to Templates?',
                    okText: 'Save draft',
                    cancelText: 'Discard & leave',
                    onOk: async () => {
                      await saveTemplate('draft')
                      navigate('/operations/surveys') // adjust to your templates index path
                    },
                    onCancel: () => {
          ...
          {provided => (
            ...
            >
              {formData.fields.length === 0 ? (
                <Card>
                  <Text type='secondary'>
                    Click “Add Field” to start. Drag to reorder. Select a field
                    to edit details in the right panel.
                  </Text>
                </Card>
              ) : (
